...
class SystemWarpHelper extends WarpHelper {
	...
	*/
	public function init() {
		...
		$this['path']->register($this['path']->path('warp:systems/joomla.1.7/menus'), 'menu');

		// set translations
		$this->language->load('tpl_warp', $this['path']->path('warp:systems/joomla.1.7'), null, true);
		...
		if ($this->application->isAdmin()) {
	        ...
	        $this['path']->register($this['path']->path('warp:systems/joomla.1.7/config'), 'config');
			...
			if ($url = $warp_xml->first('updateUrl')->text()) {
				...
				// create check urls
				$urls['tmpl'] = sprintf('%s?application=%s&version=%s&format=raw', $url, $tmpl_xml->first('name')->text().'_j17', $tmpl_xml->first('version')->text());
