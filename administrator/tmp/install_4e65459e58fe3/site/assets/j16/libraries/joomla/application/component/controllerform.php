...
class JControllerForm extends JController
{
	...
	public function cancel($key = null)
	{
		...
		if ($recordId) {
			...
			if (!J16Controller::checkEditId($context, $recordId, $this)) {
			...
			if ($checkin) {
				if ($model->checkin($recordId) === false) {
					...
					$this->setError(JText::sprintf('3JLIB_APPLICATION_ERROR_CHECKIN_FAILED', $model->getError()));
					$this->setMessage($this->getError(), 'error');
		...
		J16Controller::releaseEditId($context, $recordId, $this);
		$app->setUserState($context.'.data',	null);
		$this->setRedirect(JRoute::_('index.php?option='.$this->option.'&view='.$this->view_list.$this->getRedirectToListAppend(), false));
	...
	public function edit($key = null, $urlVar = null)
	{
		...
		if ($checkin && !$model->checkout($recordId)) {
			...
			J16Controller::holdEditId($context, $recordId, $this);
			$app->setUserState($context.'.data', null);
			$this->setRedirect('index.php?option='.$this->option.'&view='.$this->view_item.$this->getRedirectToItemAppend($recordId, $urlVar));
	...
	public function save($key = null, $urlVar = null)
	{
		...
		if ($task == 'save2copy') {
			...
			if ($checkin  && $model->checkin($data[$key]) === false) {
				...
				$this->setError(JText::sprintf('4JLIB_APPLICATION_ERROR_CHECKIN_FAILED', $model->getError()));
				$this->setMessage($this->getError(), 'error');
				$this->setRedirect('index.php?option='.$this->option.'&view='.$this->view_item.$this->getRedirectToItemAppend($recordId, $urlVar));
		...
		if ($checkin && $validData[$key] != 0 && $model->checkin($validData[$key]) === false) {
			// Save the data in the session.
			$app->setUserState($context.'.data', $validData);

			// Check-in failed, go back to the record and display a notice.
			$this->setError(JText::sprintf('1JLIB_APPLICATION_ERROR_CHECKIN_FAILED', $model->getError()));
			$this->setMessage($this->getError(), 'error');
			$this->setRedirect('index.php?option='.$this->option.'&view='.$this->view_item.$this->getRedirectToItemAppend($recordId, $key));
