...
class JoomDOCModelUpgrade extends JModel
{
                ...
                if (($response === false) || ($response->code != 200)) {
                    ...
                    if (($regInfo->code == 10) || ($regInfo->code == 20)) {
                        ...
                        if (preg_match('/([0-9]*)\/([0-9]*)\/([0-9]*)/', JString::trim($lines[1]), $match)) {
                        	$regInfo->date = JHtml::date($match[3] . '-' . $match[1] . '-' . $match[2], JText::_('DATE_FORMAT_LC3'), false);	
                        }
    ...
    function upgrade()
    {
        ...
        if (! in_array($curVersion . '.php', $upgradeFiles)) {
            ...
            $xmlFile = $package['dir'] . DS . ARTIO_UPGRADE_CAT . '.xml';
    ...
    function _getPackageFromServer($extension)
    {
        ...
        $tmp_dest = $config->getValue('config.tmp_path') . DS . ARTIO_UPGRADE_OPTION_PCKG . '.zip';
