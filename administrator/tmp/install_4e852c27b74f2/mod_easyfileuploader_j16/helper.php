...
class modEasyFileUploaderHelper
{
	...
	{
		$result = "";
		$path = JPATH_SITE.DS.'images'.DS.$params->get('efu_folder');
				...
				else
				{
					...
					if (modEasyFileUploaderHelper::isValidFileType($params, $i))
					{
						...
						if ($_FILES[$params->get('efu_variable')]["size"][$i] < $params->get('efu_maxsize'))
						{
							...
							if (file_exists($path.DS.$_FILES[$params->get('efu_variable')]["name"][$i]))
							{
								//file already exists
								...
								if ($params->get('efu_replace') == "0" && $_POST["answer"] == "0")
								{
									...
									JFile::delete($path.DS.$_FILES[$params->get('efu_variable')]["name"][$i]);
	...
	private static function storeUploadedFile($filepath, &$params, &$result, &$i)
	{
		...
		JFile::upload($_FILES[$params->get('efu_variable')]["tmp_name"][$i], $filepath.DS.$_FILES[$params->get('efu_variable')]["name"][$i]);
