		...
		<tr>
				...
				<?php foreach ($this->templates as $template) {
					...
					foreach ($renderer->getLayouts($path) as $layout) {
                        ...
                        if (in_array($metadata->get('type'), array(null, 'related', 'googlemaps'))) {
                            ...
                            // create link
                            $link = '<a href="'.$this->app->link(array('controller' => $this->controller, 'task' => 'assignelements', 'group' => $this->group, 'type' => $type->id, 'template' => $template->name, 'layout' => $layout)).'">'.$metadata->get('name', $layout).'</a>';
			...
			<td class="module">
				<?php foreach ($this->modules as $module) {
					...
					if (($xml = $this->app->xml->loadFile($module['path'].DIRECTORY_SEPARATOR.$module['name'].'.xml')) && $xml->getName() == 'install') {
						$module_name = (string) $xml->getElementByPath('name');
					}
					
					echo '<div>'.$module_name.': ';

					$renderer = $this->app->renderer->create()->addPath($module['path']);
					...
					foreach ($renderer->getLayouts('item') as $layout) {
						...
						// create link
						$link = '<a href="'.$this->app->link(array('controller' => $this->controller, 'task' => 'assignelements', 'group' => $this->group, 'type' => $type->id, 'module' => $module['name'], 'layout' => $layout)).'">'.$metadata->get('name', $layout).'</a>';
				...
				<?php 
					foreach ($this->plugins as $plugin_type => $plugins) {
						foreach ($plugins as $plugin) {
							...
							if (($xml = $this->app->xml->loadFile(dirname($plugin['path']).DIRECTORY_SEPARATOR.$plugin['name'].'.xml')) && $xml->getName() == 'install') {
								$plugin_name = (string) $xml->getElementByPath('name');
							}							
							echo '<div>'.$plugin_name.': ';
					
							$renderer = $this->app->renderer->create()->addPath($plugin['path']);
							...
							foreach ($renderer->getLayouts('item') as $layout) {
								...
								// create link
								$link = '<a href="'.$this->app->link(array('controller' => $this->controller, 'group' => $this->group, 'task' => 'assignelements', 'type' => $type->id, 'plugin' => $plugin_type.'/'.$plugin['name'], 'layout' => $layout)).'">'.$metadata->get('name', $layout).'</a>';
