...
class GWebSearch
{
	...
	{
		$lists = array();
		$lists['custom_css']			= trim( $params->get('custom_css') );
		$lists['loading']			= trim( $params->get('loading', 'Loading...') );
		$lists['controlOrder']			= trim( $params->get('order', 'custom,local,web,video,blog,news,image,book') );
		$use['customsearchengine']		= intval( $params->get('usecustomsearch', 0) );
		$expands['customexpand']		= intval( $params->get('customexpand', 1) );
		$lists['Label']				= trim( $params->get('label', 'Custom Search') );
		$lists['suffix']				= trim( $params->get('suffix') );
		$lists['siteRestriction']			= trim( $params->get('siteRestriction') );
		$use['localsearch']			= intval( $params->get('uselocalsearch', 0) );
		$expands['localexpand']		= intval( $params->get('localexpand', 1) );
		$use['videosearch']			= intval( $params->get('usevideosearch', 1) );
		$expands['videoexpand']		= intval( $params->get('videoexpand', 1) );
		$use['websearch']			= intval( $params->get('usewebsearch', 1) );
		$expands['webexpand']		= intval( $params->get('webexpand', 1) );
		$use['blogsearch']			= intval( $params->get('useblogsearch', 1) );
		$expands['blogexpand']		= intval( $params->get('blogexpand', 1) );
		$use['newssearch']			= intval( $params->get('usenewssearch', 1) );
		$expands['newsexpand']		= intval( $params->get('newsexpand', 1) );
		$use['imagesearch']			= intval( $params->get('useimagesearch', 1) );
		$expands['imageexpand']		= intval( $params->get('imageexpand', 1) );
		$use['booksearch']			= intval( $params->get('usebooksearch', 1) );
		$expands['bookexpand']		= intval( $params->get('bookexpand', 1) );
		$lists['centerpoint']			= $params->get('centerpoint');
		$lists['maxkeyword']			= intval( $params->get('maxkeyword', 5) );
		$lists['basewords']			= $params->get('searchwords', 'Joomla');
		$lists['priorityOrder']			= trim( $params->get('priority', 'content,title,page,default') );
		$lists['langJP']				= (JApplication::getCfg('language') == 'ja-JP');//for Japanese
		$lists['minlength']			= intval( $params->get('minword', 5) );
		$lists['id']				= GWebSearch::isContent();

		$lists['use'] = $use;
		$lists['expands'] = $expands;

		$Related_Search		= intval( $params->get('related_search', 0) );
