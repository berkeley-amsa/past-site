...
abstract class AppExporter {
	...
	*/
	...
	protected function _addItemCategory(AppXMLElement $item, $category, $primary = false) {
		if (!$item->categories) {
			$item->addChild('categories');
		}

		$category_xml = $item->categories[0]->addChild('category', $category, null, true);
		if ($primary) {
			$category_xml->addAttribute('primary', 'true');
		}
	}
