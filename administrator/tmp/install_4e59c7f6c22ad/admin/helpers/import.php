...
class ImportHelper extends AppHelper {
	...
	*/
	public function import(
		...
		if ($xml = $this->app->xml->loadFile($xml_file)) {
			...
			if ($application = $this->app->zoo->getApplication()) {
				...
				// import items
				$this->_importItems($application, $xml->items, $element_assignment, $types, $categories);
	...
	private function _importItems(Application $application, $items_xml_array = array(), $element_assignment = array(), $types = array(), $categories = array()) {
				...
				while ($traverse) {
					...
					foreach ($items_xml->item as $item_xml) {
						...
						foreach ($item_xml->getElementsByPath('categories/category') as $category_xml) {
							$category_alias = (string) $category_xml;
							if (isset($categories[$category_alias]) || $category_alias == '_root') {
								...
								if ($category_xml->attributes()->primary == 'true') {
									$item->getParams()->set('config.primary_category', $categories[$category_alias]->id);
								}
							}
						}

						if (!empty($item_categories)) {
							$this->app->category->saveCategoryItemRelations($item->id, $item_categories);
						}
