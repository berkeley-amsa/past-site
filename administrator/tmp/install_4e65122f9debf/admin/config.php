...
$zoo->path->register($zoo->path->path('root:cache'), 'cache');

$zoo->path->register($zoo->path->path('media:zoo/applications'), 'applications');
$zoo->path->register($path.'/assets', 'assets');

// create zoo cache folder if none existent
if (!$zoo->path->path('cache:com_zoo')) {
	JFolder::create($zoo->path->path('cache:').'/com_zoo');
}
$zoo->path->register($zoo->path->path('cache:com_zoo'), 'cache');
