...
* @copyright (c) 2009-2011 Enrico Sandoli
...
class plgContentjsmallfib extends JPlugin
{
	...
	function onPrepareContent(&$article, &$params, $limitstart = null) {
		...
		$this->DEBUG_enabled		= $this->_params->def('DEBUG_enabled', "0");

		$this->encode_to_utf8		= $this->_params->def('encode_to_utf8', "1");

		$this->display_currentdirectory = $this->_params->def('display_currentdirectory', '1');

		$this->display_filesize         = $this->_params->def('display_filesize', '1');
		$this->display_filedate         = $this->_params->def('display_filedate', '1');
		$this->date_format 	 	= $this->_params->def('date_format', 'dd_mm_yyyy_slashsep');
		$this->display_filetime 	= $this->_params->def('display_filetime', '1');
		...
		$this->filter_list_width 	= $this->_params->def('filter_list_width', '220');
		...
		$access_rights_args_found = preg_match_all("/(g?\d+|reg|thumbsize|sortby|sortas|sortnat|unzip|width):\d+n?/i", $command_match[0], $access_rights_args); 	// introduced 'n' suffix to permission
		...
		/* moved after userbound and groupbound access level check
		...
		*/
		...
		$this->default_sort_nat  = $this->_params->def('default_sort_nat', 1);
		...
		$this->unzip_allow = $this->_params->def('unzip_allow', 1);
		...
		if (stristr($this->default_absolute_path, "GROUPBOUND"))
			...
			$keyword_replacement = (strlen($groupbound_prefix) ? $groupbound_prefix." " : "").($groupbound_parameter ? $user_cattitle : $user_catid).(strlen($groupbound_suffix) ? " ".$groupbound_suffix : "");
		...
		if (strtoupper($repository) == "USERBOUND")
			...
			$userbound_parameter = $this->_params->def('userbound_parameter', 0);	// 0 for ID, 1 for NAME, 2 for USERNAME, 3 for ID USERNAME
			...
			if ($this->_params->def('default_personal_access_rights') > $access_rights)
			{
		...
		{
			if (!$user_catid)	// TODO user_catid 0 means the user is not in a group. But maybe this could be a group in itself ...
		       	{
				$text  = "<div id='error'>"
					."<table cellspacing='0' cellpadding='0' border='0' width='100%'>"
					."<tr height='30' valign='middle'>"
			...
			//peter geiger: if user already got higher rights based as a USER or GROUP he is in so be it ...
			if ($this->_params->def('default_group_access_rights') > $access_rights)
			{
		...
		$log_unzippedfiles  = $this->_params->def('log_unzippedfiles', 0);
		...
		if(isset($_GET["dir"]) && strlen($_GET["dir"])) 
		{
			...
			else
			{
				...
				else if ($this->unzip_allow && $access_rights > 2 && isset($_GET["extfile"]) && strlen($_GET["extfile"]))
				{
...
//		if(!stristr(str_replace("/", "\\", $dir), str_replace("/", "\\", $_SERVER["DOCUMENT_ROOT"]))) 
		...
		{
			$log_file = $logfile_prefix."unzippedfiles.txt";
			$log_text = JText::sprintf('unzippedfile_log_text', $today, $this->chosen_encoding($unzipped_file), $this->chosen_encoding($relative_dir), $username, $remote_address);
			if($log_unzippedfiles > 1 && $this->email_log($log_text))
			{
				file_put_contents($log_file, "MAIL ERROR > ", FILE_APPEND);
			}
			file_put_contents($log_file, $log_text, FILE_APPEND);
		...
		{
			$this->view_log($logfile_prefix, $article, $params, $description, $masked_dir, $log_uploads, $log_downloads, $log_removedfolders, $log_removedfiles, $log_restoredfiles, $log_newfolders, $log_newfoldernames, $log_newfilenames, $log_unzippedfiles);
			return;
		}
		...
		if ($this->filter_list_allow && (count($dirs) || count($files)))
		{
			$clear_current_filter_list_icon = isset($_COOKIE['current_filter_list']) && strlen($_COOKIE['current_filter_list']) ?
			       	"<a href='".$this->baselink."&dir=".urlencode($masked_dir)."&current_filter_list='><img src='".$this->imgdir."delete.png' title='".JText::_('clear_current_filter_list')."'></a>" : "";

			$filter_list_tr = $file_filter_pattern_required ? "<tr class='jsline'><td colspan='7'><img src=\"".$this->imgdir."null.gif\" /></td></tr>" : "";
			$filter_list_tr .= "<tr ".($file_filter_pattern_required ? "class='row highlighted' style='border-style:solid'" : "").">"
				...
				."<tr height='30' valign='middle'>"
					...
					."<td width='".($this->filter_list_width + 30 + (strlen($clear_current_filter_list_icon) ? 30 : 0))."' align='right'>"
						...
						."<tr>"
							...
							."</td>"
							."<td class='jsmallfibicon'>".$clear_current_filter_list_icon."</td>"
						."</tr>"
						."</table>"
					."</td>"
				."</tr>"
				...
				."</tr>";
			$filter_list_tr .= $file_filter_pattern_required ? "<tr class='jsline'><td colspan='7'><img src=\"".$this->imgdir."null.gif\" /></td></tr>"
					."<tr><td colspan='7'><img src=\"".$this->imgdir."null.gif\" height=20 /></td></tr>" : "";
		}
		else
		{
			$filter_list_tr = "";
		...
		$logs_link = $access_rights > 4 && $logs_enabled ? "<a href='".$this->baselink."&dir=".urlencode($masked_dir)."&view_log=1'>".JText::_('view_log')."</a>" : "";
		if ($show_help_link) 
		{
                ...
                if ($this->display_currentdirectory)
		{
			...
			$currentdirectory_td = "<td>".$browsing_text.": ".$this->chosen_encoding($current_position_links)."</td>";
		}
		else
		{
			$currentdirectory_td = "";
		...
		// start jsmallframe area
		$text .= "<div id='jsmallframe'>";
		...
		{
			$text .= "<td class='jsmallfibiconflex'><img src=\"".$this->imgdir."null.gif\" /></td>";
		}
		...
		if ($dirs)
		{
			foreach ($dirs as $a_dir)
			{
				...
				if ($this->line_height)
				{
					$text .= "<tr class='jsline'><td colspan='7'><img src=\"".$this->imgdir."null.gif\" /></td></tr>";
				}
				...
				if (isset($_GET['old_foldername']) && strlen($_GET['old_foldername']) && !strcmp($_GET['old_foldername'], $a_dir))   // Removed urldecode on _GET /ErikLtz
				{
					$text .= "<form action='".$this->baselink."&dir=".urlencode($masked_dir)."' method='post'>"
						...
						."	<td class='jsmallfibiconflex'><img src=\"".$this->imgdir."null.gif\" /></td>" // flexible width
						."	<td class='jsmallfibiconflex'><img src=\"".$this->imgdir."null.gif\" /></td>" // flexible width
						."	<td class='jsmallfibiconflex'><img src=\"".$this->imgdir."null.gif\" /></td>" // this covers unzip space in files
						."	<td class='jsmallfibicon'>"
			...
			foreach ($files as $a_file)
			{
				...
				if (isset($_GET['old_filename']) && strlen($_GET['old_filename']) && !strcmp($_GET['old_filename'], $a_file["name"]))   // Removed urldecode on _GET /ErikLtz
				{
					$text .= "<form action='".$this->baselink."&dir=".urlencode($masked_dir)."' method='post'>"
						...
						."	</td>"
							.($this->display_filesize ? "<td class='size'>".$this->fileSizeF($a_file["size"])."</td>" :
					       							"<td class='jsmallfibiconflex'><img src=\"".$this->imgdir."null.gif\" /></td>")
							.($this->display_filedate ? "<td class='changed'>".$this->fileChanged($a_file["changed"])."</td>" : 
					       							"<td class='jsmallfibiconflex'><img src=\"".$this->imgdir."null.gif\" /></td>")
						."	<td class='jsmallfibiconflex'><img src=\"".$this->imgdir."null.gif\" /></td>" // this covers unzip space
						."	<td class='jsmallfibicon'>"
					...
					if($access_rights > 2)
					{
						...
						if($this->unzip_allow && !$is_current_position_an_archive && in_array(strtoupper($a_file["extension"]), $supported_zip_extensions))
						{
							$text .= "<td class='jsmallfibicon'>"
								...
								."<img src=\"".$this->imgdir."unzip.png\" border='0' title=\"".JText::sprintf('extract_file', $this->chosen_encoding($a_file["name"]))."\" /></a>"
								."</td>";
						}
						else
						{
		...
		// Closing files/folders table and jsmallframe div
		$text .= "</table>"
			."</div>";
		...
		if ($access_rights > 1 && strcmp($this->baseName($dir), "JS_ARCHIVE") && (!$allow_upload_box_hiding || (isset($_COOKIE['display_actions']) && !strcmp($_COOKIE['display_actions'], "UPLOAD"))))
		{
			...
			if ($split_upload_section)
			{
				$text .= "</tr>"
					...
					if ($this->line_height)
					{
						$text .= "<tr height='5'><td><img src=\"".$this->imgdir."null.gif\" /></td></tr>"
							."<tr class='jsline'><td><img src=\"".$this->imgdir."null.gif\" /></td></tr>";
					}
					$text .= "<tr height='8'><td><img src=\"".$this->imgdir."null.gif\" /></td></tr>"
						."<tr height='20' valign='middle'>"
						."	<td>";
			}
			else
		...
		{
			// for the upload box (not allowed in archive folder)
			if ($allow_upload_box_hiding && strcmp($this->baseName($dir), "JS_ARCHIVE") && $access_rights > 1)
			{
				$upload_actions_icon = "<td class='jsmallfibicon' width='20'><a href='".$this->baselink."&dir=".urlencode($masked_dir)."&set_display_actions_cookie=UPLOAD' "
	...
	function view_log($logfile_prefix, &$article, &$params, $description, $masked_dir, $log_uploads, $log_downloads, $log_removedfolders, $log_removedfiles, $log_restoredfiles, $log_newfolders, $log_newfoldernames, $log_newfilenames, $log_unzippedfiles)
	{
		...
		if ($log_unzippedfiles) {
			...
			$logfile = $logfile_prefix."unzippedfiles.txt";
			$logtext = @file_get_contents($logfile);
	...
	function do_css()
	{
		$css = ""
			...
			."table.table td.jsmallfibiconflex {"
			."	text-align:center;"
			."	width:0px;"
			."}	"
