...
JVSlides.hslider = new Class({
	...
	initSlices: function(){
        ...
        for(var i = 0; i<this.options.slices; i++){
			...
			var sliceStyles = {
				top: Math.round(that.options.height/that.options.slices) * i,
				left: 0,
				width: 0,
				height: (i == this.options.slices - 1) ? that.options.height - (Math.round(that.options.height/that.options.slices) * i) : Math.round(that.options.height/that.options.slices)
			};
	...
	fold: function(){		
		...
		this.items.getElements('.slice').each(function(sliceElement, index){
			var sliceFx = {
				opacity: 1,
				height: sliceElement.getCoordinates().height				
			};			
			sliceElement.setStyles({
				width: this.options.width,
				height: 0
