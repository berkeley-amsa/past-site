...
switch ($jvslideshow_source){
    case 0:
        ...
        break;
    case 1:
        $items = json_decode($params->get('JVSlideshow_slides', '[]'),true);
		
        foreach($items as $index =>$item){
            $items[$index]['name'] = $jvslideshow_imagespath.$item['image'];
            $items[$index]['description'] = $item['desc'];
			unset($items[$index]['image']);			unset($items[$index]['desc']);
	
        }
        //ModJVSlideShowHelper::getImageLists($jvslideshow_images, $jvslideshow_titles, $jvslideshow_description, $jvslideshow_links, $jvslideshow_imagespath);
        break;
    default:
