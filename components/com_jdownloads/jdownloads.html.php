...
############################################################## */
    ...
    if (!$cat[0]->metadesc){
        $document->setMetaData( 'keywords' , $jmetadesc);
    } else {
        $document->setMetaData( 'keywords' , strip_tags($cat[0]->metadesc));
    }    
...
function showDownload($option, $file, $cat, $access, $edit_link, $user_can_edit){
    ...
    if (!$file->metadesc){
        ...
        $document->setMetaData( 'keywords' , strip_tags($file->metadesc));
    }     
    ...
    if (!$file->description_long){
        ...
        // support for content plugins
        if ($jlistConfig['activate.general.plugin.support'] && $jlistConfig['use.general.plugin.support.only.for.descriptions']) {  
            $file->description_long = JHTML::_('content.prepare', $file->description_long);
        }
...
function viewUpload($option, $view ){
    ...
    if (intval($jlistConfig['upload.access.group']) > 0){ 
        ...
        if ($aid < (int)$jlistConfig['upload.access']){
